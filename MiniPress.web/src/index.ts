...
function displayArticlesFromCache() {
    ...
    const articlesFiltresParMotCle = cachedArticles.filter(art => {
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #eee; margin-bottom: 1rem; flex-wrap: wrap; gap: 1rem;">
            ...
            <div style="display: flex; gap: 1rem; align-items: center; flex-wrap: wrap;">
                <div>
                    ...
                    <input type="text" id="searchInput" value="{{keyword}}" placeholder="Titre ou resume..." style="padding: 0.3rem; width: 200px;">
                </div>
                <div>
                    <label for="sortOrder"><strong>Trier : </strong></label>
                    <select id="sortOrder" style="padding: 0.3rem;">
                        <option value="desc" {{#if isDesc}}selected{{/if}}>Plus recents</option>
...
async function renderArticles(fetchPromise: Promise<any[]>, sourceKey: string) {
    const appContainer = document.getElementById('app');
    if (!appContainer) return;

    if (lastFetchedSource === sourceKey && cachedArticles.length > 0) {
...
function handleNavigation() {
    ...
    if (hash.startsWith('#/categories/')) {
        ...
        renderArticles(fetchArticlesByCategorie(paramCat), `cat_${paramCat}`);
    } 
    else if (hash.startsWith('#/auteurs/')) {
        const idAuteur = hash.replace('#/auteurs/', '');
        renderArticles(fetchArticlesByAuteur(idAuteur), `auteur_${idAuteur}`);
    }
        ...
        renderArticles(fetchArticles(), 'all');
    }
}
...
document.addEventListener('input', (event) => {
    ...
    if (target && target.id === 'searchInput') {
        currentKeyword = (target as HTMLInputElement).value;
        displayArticlesFromCache(); 
    }
});
...
document.addEventListener('change', (event) => {
    const target = event.target as HTMLElement;
    if (target && target.id === 'sortOrder') {
        currentSortOrder = (target as HTMLSelectElement).value as 'asc' | 'desc';
        displayArticlesFromCache(); 
    }
});
